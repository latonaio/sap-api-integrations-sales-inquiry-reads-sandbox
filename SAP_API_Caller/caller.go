...
func (c *SAPAPICaller) AsyncGetSalesInquiry(salesInquiry, salesInquiryItem string) {
	wg := &sync.WaitGroup{}
...
func (c *SAPAPICaller) callSalesInquirySrvAPIRequirementHeader(api, salesInquiry string) ([]sap_api_output_formatter.Header, error) {
	...
	c.getQueryWithHeader(req, salesInquiry)
...
func (c *SAPAPICaller) callSalesInquirySrvAPIRequirementItem(api, salesInquiry, salesInquiryItem string) ([]sap_api_output_formatter.Item, error) {
	...
	c.setHeaderAPIKeyAccept(req)
	c.getQueryWithItem(req, salesInquiry, salesInquiryItem)
...
func (c *SAPAPICaller) getQueryWithHeader(req *http.Request, salesInquiry string) {
	params := req.URL.Query()
	params.Add("$filter", fmt.Sprintf("SalesInquiry eq '%s'", salesInquiry))
	req.URL.RawQuery = params.Encode()
}

func (c *SAPAPICaller) getQueryWithItem(req *http.Request, salesInquiry, salesInquiryItem string) {
	params := req.URL.Query()
	params.Add("$filter", fmt.Sprintf("SalesInquiry eq '%s' and SalesInquiryItem eq '%s'", salesInquiry, salesInquiryItem))
	req.URL.RawQuery = params.Encode()
}
